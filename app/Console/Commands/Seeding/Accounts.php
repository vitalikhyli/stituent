...
class Accounts extends Command
{
     ...
     * @var string
     */
    protected $signature = 'st:seed_accounts';
    ...
    public function handle()
    {
        ...
        if (! $this->confirm('ACCOUNTS: Do you wish to continue?')) {
            return;
        }

        echo date('Y-m-d h:i:s')." Starting Accounts\n";
        ...
        $expected = $campaigns->count();
        $current = 0;

        foreach ($campaigns as $campaign) {
            echo '**** Account '.$current++.' of '.$expected."         \r";
        ...
        $expected = $cc_users->count();
        $current = 0;

        foreach ($cc_users as $cc_user) {
            echo '**** Users '.$current++.' of '.$expected."          \r";

            $user = User::where('username', $cc_user->login)->first();
            $office_team = Team::where('old_cc_id', $cc_user->campaignID)->first();
            if (! $office_team) {
            ...
            if (! $user) {
                ...
                if ($cc_user->email) {
                    ...
                    $user->email = 'placeholder.'.$cc_user->userID.'@stituent.com';
                }
