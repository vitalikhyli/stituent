...
class Northeastern extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'st:seed_nu';
    ...
    //if (env('LOCAL_MACHINE') != 'Slothe')             return;
        if (! $this->confirm('NU: Do you wish to continue?')) {
            return;
        }

        echo date('Y-m-d h:i:s')." Starting Northeastern\n";
        ...
        foreach ($ptypes as $ptype) {
            ...
            $partnership_type->team_id = $team->id;
            $partnership_type->user_id = $user->id;
        ...
        while ($row = fgetcsv($ccfile, 0, '|')) {
            ...
            $entity = Entity::where('team_id', $team->id)
            ...
            foreach ($programs as $key => $program) {
                $pt_id = PartnershipType::where('team_id', $team->id)
                                ->where('name', 'Co-Curricular')
                                ->first()->id;

                $pship = new Partnership;
                $pship->team_id = $team->id;
                $pship->user_id = $user->id;
        ...
        while ($row = fgetcsv($servicelearningfile, 0, '|')) {
            ...
            $service_learning->team_id = $team->id;
            $service_learning->user_id = $user->id;
    ...
    public function seedFindOrCreate($entity_or_person, $name, $contact_info, $entity_type = null)
    {
        ...
        if ($entity_or_person == 'entity') {
            if (Entity::where('name', $name)->where('team_id', $team->id)->doesntExist()) {
                $thing = new Entity;
                $thing->team_id = $team->id;
            ...
            //echo 'CREATED '.$entity_or_person.' ('.$entity_type.') '.$thing->name." \r\n";
                ...
                $thing = Entity::where('name', $name)->where('team_id', $team->id)->first();
            }
        }
        if ($entity_or_person == 'person') {
            if (Person::where('full_name', $name)->where('team_id', $team->id)->doesntExist()) {
                $thing = new Person;
                $thing->team_id = $team->id;
            ...
            //echo 'CREATED '.$entity_or_person.' (person) '.$thing->full_name." \r\n";
                ...
                $thing = Person::where('full_name', $name)->where('team_id', $team->id)->first();
            }
        }
    ...
    public function seedLink($subject, $subject_type, $kind, $object, $object_type)
    {
        $team = Team::where('old_cc_id', 409)->first();
        $user = User::where('username', 'disberg')->first();
        if (Relationship::where('team_id', $team->id)
                        ->where('kind', $kind)
