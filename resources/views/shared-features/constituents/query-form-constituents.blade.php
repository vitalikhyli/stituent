...
<form class="px-2" id="constituents-list-form" style="display:none;" action="" method="GET" autocomplete="off" autocomplete="false">
	...
	@if(!Auth::user()->team->db_slice)
 		...
 		<input query_form="true" type="checkbox" name="linked" checked />

	@else

		<div class="flex w-full items-center text-sm uppercase text-grey font-bold -mt-2">
			<div class="w-1/3 text-right whitespace-no-wrap text-xs">
			...
			<div class="w-1/3 text-left text-xs">
				Linked Only
			</div>
		</div>
	...
	<div class="flex w-full">
		...
		@if(isset($input['last_name']) && $input['last_name'])
			<div class="w-1/6" id="middle_name_div">
				<input query_form="true" type="text"
					   name="middle_name" 
					   id="middle_name" 
					   autocomplete="off"
					   value="{{ (isset($input['middle_name'])) ? $input['middle_name'] : '' }}"
					   placeholder="Middle" 
					   class="w-full appearance-none px-1 py-3 bg-grey-lighter border-t-2 border-b-2 border-r-2 border-grey text-black focus:border-2 text-lg" />
			</div>
		@endif
	...
	<div class="py-2 px-4 text-grey-dark">

		@if(\Carbon\Carbon::parse('2020-01-24')->diffInDays() < 14)
			<div class="text-blue-dark py-1 text-sm">
				<i class="fas fa-star"></i> New! - Email search
			</div>
		@endif
	...
	<div class="flex px-4 py-2 items-center w-full">
		<div class="w-full">
			<select query_form="true" name="per_page" class="form-control">
				@foreach([100, 200, 400, 500] as $amount)
					<option {{ selectedIfValueIs($amount, $input, 'per_page') }} value="{{ $amount }}">
