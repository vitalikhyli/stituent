	...
	@foreach($people as $theperson)
		...
		@elseif($theperson->person)
			<tr class="border-b hover:bg-orange-lighter cursor-pointer bg-orange-lightest">
				<td class="p-2 w-10 whitespace-no-wrap" valign="top">
					<span data-entity_id="" data-person_id="{{ $theperson->id }}" data-name="{{ $theperson->full_name }}" class="lookup-search-result bg-grey-lighter rounded-full m-1 px-2 py-1 text-sm border shadow">
		...
		@else
			<tr class="border-b hover:bg-blue-lightest cursor-pointer">
				...
				<td class="p-2 w-10 whitespace-no-wrap" valign="top">
					<span data-entity_id="" data-person_id="{{ $theperson->id }}" data-name="{{ $theperson->full_name }}" class="lookup-search-result bg-grey-lighter rounded-full m-1 px-2 py-1 text-sm">
						{{ $theperson->full_name }}
					</span>
				</td>
				<td class="p-2 w-10 whitespace-no-wrap overflow-x-hidden" valign="top">
