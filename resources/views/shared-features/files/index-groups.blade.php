...
<div class="w-full" id="paginate-div">
	@if($groups instanceof \Illuminate\Pagination\LengthAwarePaginator)
		<div class="w-full flex text-grey-dark">
			<div class="w-1/4 text-left p-2">
				Showing <span class="font-bold text-black">{{ $groups->firstItem() }}-{{ $groups->lastItem() }}</span> of 
				<span class="font-bold text-black">{{ $groups->total() }}</span>
			</div>
			<div class="w-1/2 text-center">
				{{ $groups->links() }}
			</div>
			<div class="w-1/4 text-right p-2">
				Page <span class="text-black font-bold">{{ $groups->currentPage() }}</span> of <span class="text-black font-bold">{{ $groups->lastPage() }}</span>
			</div>
		</div>
	@endif
</div>
	...
	@include('shared-features.files.search-bar', ['models' => $groups])
	...
	@else
		...
		<div id="list" class="mt-4 mb-4 w-full">
			...
			@include('shared-features.files.list-groups')
...
<script type="text/javascript">
	...
	function getSearchData(v) {
		...
		$.get('/{{ Auth::user()->team->app_type }}/files/search/groups/'+v+'/'+return_string, function(response) {
			$('#list').html(response);
		});
	}
