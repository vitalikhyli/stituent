...
@section('main')
	...
	<div class="md:flex pb-8 text-grey-dark">
		...
		<div class="w-3/5 mr-12 px-8">
		    ...
		    @if($cases_total >0)
				@if ($case_type == 'user')
					<a href="/office/cases" class="hover:text-blue-darker float-right text-sm text-blue rounded-full px-2 py-2">
			        	See All {{ $cases_total }}... 
			        </a>
					        	...
					        	<div class="w-full">
						        	...
						        	@elseif($contact->entities()->first())
						        		...
						        		@foreach($contact->entities as $entity)
						        			<a class="text-grey-darker hover:text-black" href="/u/entities/{{ $entity->id }}">{{ $entity->name }}</a>{{ (!$loop->last) ? ', ' : '' }}
						        		@endforeach
						        		</span>
						        	@else
		    ...
		    <div class="mt-8 mb-8">
				...
				@include('u.dashboard.organizations')
