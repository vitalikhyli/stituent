...
@section('breadcrumb')
  ...
  > <a href="/u/entities/{{ $entity->id }}">{{ $entity->name }}</a> 
  > <b>Relationship</b>
...
<form method="POST" 
  @if($mode == 'save')
    action="{{dir}}/relationships/{{ $entity->id }}/save/e2e"
  @endif
  @if($mode == 'update')
...
<div class="text-2xl font-sans border-b-4 border-blue pb-2">
    ...
    <div class="float-right text-base">
      ...
      <input autocomplete="off" type="submit" name="update" value="Save" class="rounded-lg px-4 py-2 bg-blue text-white text-center"/>
      ...
      @if($mode == 'update')
        <input type="submit" formaction="{{dir}}/relationships/{{ $relationship->id }}/update/close" name="update" value="Save and Close" class="rounded-lg px-4 py-2 bg-grey-darkest text-white text-center"/>
      @endif
...
<script type="text/javascript">
  ...
  function getSearchData_Kinds(v) {
    ...
    $.get('{{dir}}/relationships/search_kinds/e2e/'+v, function(response) {
      if (response == '') {
        $('#list-kinds').addClass('hidden');
      } else {
